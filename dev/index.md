...
![](reference/figures/to-cor-df.png)
...
Using `corrr` typically starts with
...
- [`rplot()`](https://corrr.tidymodels.org/dev/reference/rplot.md) the
  correlations with shapes in place of the values.
- [`network_plot()`](https://corrr.tidymodels.org/dev/reference/network_plot.md)
  the correlations in a network.

## Databases and Spark

The
[`correlate()`](https://corrr.tidymodels.org/dev/reference/correlate.md)
function also works with database tables. The function will
automatically push the calculations of the correlations to the database,
collect the results in R, and return the `cor_df` object. This allows
for those results integrate with the rest of the `corrr` API.
...
![](reference/figures/README-combination-1.png)
...
![](reference/figures/README-combination-2.png)
